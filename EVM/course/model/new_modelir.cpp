...
uchar IR, //регистр команд
  ...
  Ram[256],
  Xdata[1024],
  DPH, DPL, // DPTR = DPH.DPL
  P0 = 0xff, P1 = 0xff, P2 = 0xff, P3 = 0xff, //регистр портов I/O
...
uchar WRSFR[128]; // Теневые регистры?
...
struct mik16
{                 //структура микрокоманды управления блоком 16mem
  uchar selpc;    // 3
  uchar unicod16; // 6
  uchar selaxa;   // 1
...
void
TakeCode1(char* stpole, char* stmo)
{
...
void
TakeCode3(char* stpole, char* stmo)
{
...
char* Mop = "L,Suba,Subb,Add,Or,Xor,And,H "; // Микрооперации.
...
//сформировать коды в полях *stpole  для микрооперации *stmo
void
TakeCode4(char* stpole, char* stmo)
{ 
...
void
TakeCode2(char* stpole, char* stmo)
{
...
struct BUSS8
{                //структура микрокоманды управления блоком ralu
  uchar selbusb; // 4
  uchar selbusa; // 3
  uchar asfr;    // 7
  uchar unibus8; // 4
...
void
TakeCode7(char* stpole, char* stmo)
{ //сформировать коды в полях *stpole  для микрооперации *stmo
...
//                   1      2     3   4     5      6
void
TakeCode8(char* stpole, char* stmo)
{
...
void
Takeromm(char* stpole, char* stmo)
{ //сформировать коды в полях *stpole  для микрооперации *stmo
...
void
CompareCode(void) //формирование кодов микрокоманд по блокам
{
  ...
  CodDCM4 =
    (((((((((alu.mop << 3) | alu.selacc) << 2) | alu.selb) << 1) | alu.selpa)
       << 1) |
      alu.selpb)
     << 9) |
    alu.uniralu;
...
char
odd(void) //формирование признака четности P
{
  char yy;
  char x0 = ACC & 1;
  char x1 = ACC & 2;
...
{ //функция клавиши  Сброс
  ...
  for (i = 0x28; i < 0x2f; i++)
    ADC[i] = j;
  j++; // j=5 команда add a,ri

  for (uchar i = 0x11; i < 0xF1; i = i + 0x10)
    ADC[i] = j;
  j++; // j=6 команды acall met
    ...
    case 8: // reti
      ...
      case 11: // orl c, bit
      {
        { // Reading the bit address
          ...
          ss[0] = (Wrk & 0x80) | 0x30;
          ss[1] = '\0';
          char code[12] = "orl c, "; // actually a lie
          Instr->Text = StrCat(code, ss);
        }
      ...
      case 13: // mov a, @rj
      {
        ...
        }
        {
          Ram[Acc] = ACC = Ram[Wrk];
          RAMK++;
          PSWC(ss); //формирование признаков
      ...
      case 16: // jb bit, rel
      {
        ...
        }
        {
          ...
          ss[0] = (Wrk & 0x7) | 0x30;
          ss[1] = '\0';
          char code[12] = "jb Acc."; // actually a lie
          Instr->Text = StrCat(code, ss);
        }
        { // 7.2. Reading from SFR or RAM
          if (Wrk & 0x80)
...
void
Creatwrsfr(void) //декодер адресов SFR в wrsfr в проекте
{
  WRSFR[Sp & 0x7f] = 1;
  WRSFR[Acc & 0x7f] = 2;
  WRSFR[Dph & 0x7f] = 4;
...
uchar VECTOR[16];
void
Creatvector(void) //вектора прерываний
{
